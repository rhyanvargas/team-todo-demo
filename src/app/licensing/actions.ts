...
const baseUrl = process.env.NEXT_PUBLIC_NGROK_URL || "http://localhost:3005";

export async function toggleUserLicense(
	orgId: string,
	userId: string,
	status: boolean
) {
...
export async function getCheckoutUrl(clerkOrgId: string, quantity: number) {
	...
	const session = await stripe.checkout.sessions.create({
		...
		success_url: `${baseUrl}/licensing`,
		cancel_url: `${baseUrl}/licensing`,
	});
	return session.url;
}
...
export async function getPortalUrl(clerkOrgId: string) {
	...
	if (!stripeId) throw new Error("No Stripe customer found");

	const session = await stripe.billingPortal.sessions.create({
		customer: stripeId,
		return_url: `${baseUrl}/licensing`,
	});
	return session.url;
}
